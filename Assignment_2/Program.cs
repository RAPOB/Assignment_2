...
namespace Assignment_2
{
    class FiniteStateMachine // general class for both Finite State Machines (FSM)
    {
        ...
        public void setnextState(int d_state, int x, int y) { this.FST[x, y].nextState = d_state; }                                          
        public void setAction(string d_action, int x, int y) { this.FST[x, y].action = d_action; }
        public int getnextState(int x, int y) { return this.FST[x, y].nextState; }
        public string getAction(int x, int y) { return this.FST[x, y].action; }
        ...
        public void running(string stuff) // method to run other methods based on a string input pulled from the FST
        {
            ...
            if (stuff != "J K L")
            {
                ...
                {
                    MethodInfo run = this.GetType().GetMethod(values[i]); //runs the associated methods using the inbuilt MethodInfo class                                                                          
